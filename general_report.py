...
def count_refurbished_products(df):
    ...
    return (df.loc[df['Product_State'] == '\
    refurbished']).Product_Quantity.sum()
...
def general_data_table(df):
    ...
    fig = go.Figure(data=[go.Table(header=dict(
        values=['Description:', 'Results']), cells=dict(values=[
            [
                "Total Orders",
                "Rentals",
                "Sales",
...
def general_data_donutChart(df):
    ...
    fig = px.pie(
        data,
        values='Order_no', names='Finance_Type',
        color='Finance_Type',
        color_discrete_map={'Sales': '#003f5c', 'Rentals': '#ffa600'}
    )
    fig.update_traces(
        hole=.4, textinfo='value + percent',
        hovertemplate="%{label}<br>Total Orders: %{value} \
...
def general_bubble_chart(df):
    ...
    fig = px.scatter(
        df, x="Date", y="Business_Sector", color="Finance_Type",
        size='Product_Quantity',
        title="Scatter Chart: Sales vs. Rentals",
        height=800
    )
    return fig.to_html(full_html=True, include_plotlyjs='cdn')
...
def general_Analysis(df):
    """Master function to generate all functions \
    above and add them to the file """
