...
def product_treemap(df):
    ...
    fig = go.Figure(go.Treemap(
        ...
        parents=['Makes'] * len(product_type_counts),
        # Set parent as 'Products' for all labels
...
def product_pie_chart(df):
    ...
    grouped_data = df.groupby(
        ['Product_Make', 'Product_Type']
    )['Product_Quantity'].sum().reset_index()
    sum_product_quantity = grouped_data.groupby(
        'Product_Make')['Product_Quantity'].sum()

    ordered_df = grouped_data.merge(
        sum_product_quantity, left_on='Product_Make', right_index=True
    )
    ordered_df = ordered_df.sort_values(
        by='Product_Quantity_y', ascending=False
    ).drop('Product_Quantity_y', axis=1)

    # Check the number of unique product makes to
    # ensure I select the right function to call
...
def top_four_pie_product(ordered_df):
    best_product_1 = ordered_df[ordered_df[
        'Product_Make'] == ordered_df['Product_Make'].unique()[0]]
    best_product_2 = ordered_df[ordered_df[
        'Product_Make'] == ordered_df['Product_Make'].unique()[1]]
    best_product_3 = ordered_df[ordered_df[
        'Product_Make'] == ordered_df['Product_Make'].unique()[2]]
    best_product_4 = ordered_df[ordered_df[
        'Product_Make'] == ordered_df['Product_Make'].unique()[3]]
    ...
    fig.add_trace(
        go.Pie(
            labels=best_product_1['Product_Type'],
            values=best_product_1['Product_Quantity_x'],
            name=ordered_df['Product_Make'].unique()[0]
        ), row=1, col=1
    )
    fig.add_trace(
        go.Pie(
            labels=best_product_2['Product_Type'],
            values=best_product_2['Product_Quantity_x'],
            name=ordered_df['Product_Make'].unique()[1]
        ), row=1, col=2
    )
    fig.add_trace(
        go.Pie(
            labels=best_product_3['Product_Type'],
            values=best_product_3['Product_Quantity_x'],
            name=ordered_df['Product_Make'].unique()[2]
        ), row=2, col=1
    )
    fig.add_trace(
        go.Pie(
            labels=best_product_4['Product_Type'],
            values=best_product_4['Product_Quantity_x'],
            name=ordered_df['Product_Make'].unique()[3]
        ), row=2, col=2
    )
    ...
    fig.update_layout(
        title_text='Top Products by Brand Pie Charts',
        showlegend=True,
        annotations=[
            dict(
                text=ordered_df['Product_Make'].unique()[0],
                font_size=16, showarrow=False
            ),
            dict(
                text=ordered_df['Product_Make'].unique()[1],
                showarrow=False, font_size=16
            ),
            dict(
                text=ordered_df['Product_Make'].unique()[2],
                showarrow=False, font_size=16
            ),
            dict(
                text=ordered_df['Product_Make'].unique()[3],
                showarrow=False, font_size=16
            )
        ]
    )
...
def top_two_pie_product(ordered_df):
    ...
    # Update layout and title
    fig.update_layout(
        title_text='Top Products by Brand Pie Charts', showlegend=True,
        annotations=[
            dict(
                text=ordered_df['Product_Make'].unique()[0],
                font_size=16, showarrow=False),
...
def top_one_pie_product(ordered_df):
    best_product = ordered_df[
        ordered_df['Product_Make'] == ordered_df['Product_Make'].unique()[0]]
    ...
    fig.add_trace(
        go.Pie(
            labels=best_product['Product_Type'],
            values=best_product['Product_Quantity_x'],
            name=ordered_df['Product_Make'].unique()[0]
        ), row=1, col=1)
...
def product_bar_chart(df):
    ...
    cap_prod_type = df.groupby(
        ['Product_State', 'Product_Type']
    )['Product_Quantity'].sum().reset_index()
    ...
    fig = go.Figure(data=[
        go.Table(
            header=dict(values=['Product Type', 'Total Sold']),
            cells=dict(values=[
                cap_prod_type.sort_values(
                    'Product_Quantity',
                    ascending=False
                ).head(n=5).Product_Type,
                cap_prod_type.sort_values(
                    'Product_Quantity',
                    ascending=False
                ).head(n=5).Product_Quantity
            ]))
    ])
...
def product_scatter(df):
    ...
    fig = px.scatter(
        scatter_sale_order, x='Date', y='Product_Make',
        color='Product_Type',
...
def product_information(df):
    ...
    fig = make_subplots(
        rows=4, cols=3,
        subplot_titles=[
            "Q1 Makes", "Q1 Product Types", "Q1 Product Condition",
            "Q2 Makes", "Q2 Product Types", "Q2 Product Condition", "Q3 Makes",
            "Q3 Product Types", "Q3 Product Condition",
    ...
    for quarter in range(1, 5):
        ...
        product_type_counts = df[
            df['Quarter'] == quarter]['Product_Make'].value_counts()

        # Create tree chart
        tree_chart = go.Treemap(
            labels=product_type_counts.index,
            parents=['Makes'] * len(product_type_counts),
            values=product_type_counts.values
        )
        ...
        bar_chart = go.Bar(
            x=bar_data['Product_State'],
            y=bar_data['Product_Quantity'],
            text=bar_data['Product_Quantity'],
            textposition='auto',
            name=f"Q{quarter}"
        )
...
def product_Analysis(df):
    """Master function to generate call functions above, \
    stored them in a string and return them"""
